---
title: "Plan and Build: thcode's Two Work Modes"
sidebarTitle: "Work Modes"
description: "thcode's Plan and Build modes are structurally orthogonal. Plan mode enforces read-only at the architecture level — no profile or override can authorize mutations."
---
...
<Warning>
  Plan mode cannot write files, delete files, run commands, or perform transfers. These are mutating action classes and are structurally denied — not just policy-denied. No profile or developer override can change this while Plan mode is active.
</Warning>
...
| `transfer`   | No       | **Yes**   | high   | `network-transfer`              | ❌ No (sensitive) |

<Note>
  `transfer` is marked non-mutating but sensitive, and carries a high risk rating. It is blocked in Plan mode not because it is mutating, but because sensitive actions always require approval unless `full-access` holds the explicit `sensitiveOverride` flag — see [Permission Profiles](/concepts/permission-profiles).
</Note>
